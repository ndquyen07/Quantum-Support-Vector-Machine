...
class QuantumKernel:
    ...
    def compute_kernel_matrix_with_inner_products(self, X1, X2, theta_params, circuit):
        """
    ...
    def compute_kernel_matrix_by_adjoint_method(self, X1, X2, theta_params, circuit, shots=1024):
        """
        ...
        for i in range(n_samples):
            for j in range(n_samples):
                ...
                param_dict_j = {}
                for k in range(X1.shape[1]):  # Fixed: use X1, not X2
                    param_dict_j[circuit.parameters[k]] = X1[j, k]
                for k, t in enumerate(theta_params):
                    param_dict_j[theta_params_list[k]] = t
    ...
    def compute_kernel_matrix_swap_test(self, X1, X2, theta_params, circuit, shots=1024):
        """
    ...
    def plot_kernel_matrix(self, kernel_matrix, title="Quantum Kernel Matrix", filename=None, cmap='Greys'):
    ...
    def plot_multi_kernel_matrices(self, matrices, titles, filename=None, cmap='Greys'):
...
if __name__ == "__main__":
    ...
    # Create a simple quantum circuit with parameters
    x1 = Parameter('x1')
    x2 = Parameter('x2')
    ...
    # Sample data
    X_sample = np.array([[0, 0], [np.pi/2, np.pi/2], [np.pi, np.pi]])
    theta_params_sample = [np.pi/4, np.pi/4]

    # Compute kernel matrix with all three methods
    kernel = QuantumKernel(X_sample, X_sample, qc, theta_params_sample)
    K1 = kernel.compute_kernel_matrix_with_inner_products()
    K2 = kernel.compute_kernel_matrix_by_adjoint_method(shots=4096)
    K3 = kernel.compute_kernel_matrix_swap_test(shots=4096)
    
    print("Kernel Matrix (Inner Products - Exact):\n", K1)
    print("\nKernel Matrix (Adjoint Method - Real States Only):\n", K2)
    print("\nKernel Matrix (Swap Test - Exact):\n", K3)
    
    # Compare methods
    print(f"\nDifference (Inner Products vs Adjoint): {np.max(np.abs(K1 - K2)):.6f}")
    print(f"Difference (Inner Products vs Swap Test): {np.max(np.abs(K1 - K3)):.6f}")