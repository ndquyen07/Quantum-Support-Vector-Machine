...
# Load data from digits_8features_01_data.npz
loaded_data = np.load('data/digits_8features_01_data.npz')
...
def main():
    """
    Main training function for TQFM on Cancer dataset
    ...
    results_dir = "results_tqfm_digits"
    ...
    model_filename = f"{results_dir}/tqfm_digits_depth{args.depth}_ansatz{args.type_ansatz}_iter{args.maxiter}_run{args.run_id}.pkl"
    ...
    kernel_filename = f"{results_dir}/kernels_digits_depth{args.depth}_ansatz{args.type_ansatz}_iter{args.maxiter}_run{args.run_id}.npz"
    np.savez(
        kernel_filename,
